#!/usr/bin/env python3
"""Render the bare Crimea panel placed beside Ukraine and Georgia."""
...
from extract_crimea_density import BBOX, NBINS
from plot_helpers import add_osm_basemap, draw_geojson_layer


ROOT = Path(__file__).resolve().parents[1]
DATA = ROOT / "data" / "out-of-country" / "additional-cases" / "crimea-russia-density-grid.csv"
OUTPUT = ROOT / "paper" / "figs" / "crimea_russia_density.png"
ADMIN1 = ROOT / "data" / "reference" / "ne_10m_admin_1_crimea.geojson"
...
def load_grid() -> tuple[np.ndarray, np.ndarray]:
    ...
    ukrainian = np.zeros((NBINS, NBINS), dtype=float)
    with DATA.open(newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            iy, ix = int(row["iy"]), int(row["ix"])
            russian[iy, ix] = int(row["russian_cells"])
            ukrainian[iy, ix] = int(row["ukrainian_cells"])
    return russian, ukrainian


def density_rgba(russian: np.ndarray, ukrainian: np.ndarray) -> np.ndarray:
    stack = np.stack([russian, ukrainian])
    ...
    cmaps = [ramp("ru", "#edb6bd", RU_COLOR), ramp("ua", "#b7cee0", DOMESTIC_COLOR)]
...
def render(output: Path = OUTPUT) -> None:
    ...
    draw_geojson_layer(ax, ADMIN1, BBOX, facecolor="none", edgecolor="#292724",
                       linewidth=0.60, alpha=0.92, zorder=2.2)
    ax.imshow(density_rgba(russian, ukrainian), origin="lower", extent=BBOX,
              interpolation="nearest", aspect="auto", zorder=3)
    # Restroke the peninsula above the dense network layer.
    draw_geojson_layer(ax, ADMIN1, BBOX, facecolor="none", edgecolor="#292724",
                       linewidth=0.42, alpha=0.90, zorder=4)
